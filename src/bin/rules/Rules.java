...
public class Rules {
    ...
    private boolean isProgram() throws Exception {
        ...
        if(!listIteratorsTokens.next().getItem().equals("program_var")){
            throw new Exception("Программа должна начинаться с program var");
        }
    ...
    private boolean isDescription() throws Exception {
        ...
        boolean varStart = listIteratorsTokens.next().getItem().equals("begin");
        ...
        if(!varStart)
            throw new Exception("Описание должно начинаться с begin!");
    ...
    private boolean isConditionalOperator() throws Exception {
        ...
        if (typesData.equals(TypesData.NON_DATA) | !typesData.equals(TypesData.BOOL)){
            throw new Exception("После оператора if должно быть выражение типа bool!");
        }
        skipLineTranslation();
    ...
    private boolean isOperatorLoop() throws Exception {
        ...
        if (typesData.equals(TypesData.NON_DATA)|typesData.equals(TypesData.BOOL)) {
            throw new Exception("После оператора for должен идти оператор присваивания (и не bool)!");
        }
        skipLineTranslation();
        ...
        skipLineTranslation();
        if (typesData.equals(TypesData.NON_DATA)|typesData.equals(TypesData.BOOL)){
            throw new Exception("После служебного слова to должно идти выражение (и не bool!)!");
        }
        skipLineTranslation();
    ...
    private TypesData isFactor() throws Exception {
        ...
        if (token.getTableIndex().equals(IndicesTables.INDICES)){
            ...
            return token.getTypeData();
        }else if (token.getItem().equals("and") && (isFactor().equals(TypesData.BOOL))){
            return token.getTypeData();
        }else {
            typesData = isExpression();
